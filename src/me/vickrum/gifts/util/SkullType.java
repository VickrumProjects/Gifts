...
public enum SkullType {
	...
	private SkullType(EntityType type, String name, String textures) {
		this.type = type;
		this.name = name;
		...
		if (textures != null) {
			...
			this.profile.getProperties().put("textures", new Property("textures", textures));

			try {
				Field field = meta.getClass().getDeclaredField("profile");
				field.setAccessible(true);
				field.set(meta, this.profile);
	...
	public static SkullType from(EntityType type) {
		SkullType[] var1 = values();
		int var2 = var1.length;

		for (int var3 = 0; var3 < var2; ++var3) {
			SkullType skull = var1[var3];
		...
		if (item.getType() == Material.SKULL_ITEM) {
			...
			try {
				...
				if (profile != null) {
					...
					SkullType[] var5 = values();
					int var6 = var5.length;

					for (int var7 = 0; var7 < var6; ++var7) {
						SkullType skull = var5[var7];
	...
	private static String getTextures(GameProfile profile) {
		...
		Iterator<Property> var2 = properties.get("textures").iterator();
		...
		} while (!property.getName().equals("textures"));
